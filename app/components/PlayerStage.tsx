...
type Props = {
  ...
  videoDuration: number; onJumpToTime: (time: number) => void;
  ...
  return (
    <section className="stage">
      ...
      <div className="player-wrap">
        ...
        <video ref={props.videoRef} className={props.activeFile ? "visible" : ""} controls playsInline onLoadedMetadata={props.onLoaded} onTimeUpdate={event => props.onTimeUpdate(event.currentTarget.currentTime)} onPause={props.onPause} onEnded={props.onEnded} />
        {props.activeFile && <NotesPanel activeFile={props.activeFile} duration={props.videoDuration} onJump={props.onJumpToTime} progress={props.progress} notes={props.notes} bookmarks={props.bookmarks} onShowNotes={props.onShowNotes} onAddBookmark={props.onAddBookmark} onAddNote={props.onAddNote} onEditBookmark={props.onEditBookmark} onDeleteBookmark={props.onDeleteBookmark} onEditNote={props.onEditNote} onDeleteNote={props.onDeleteNote}/>} 
      </div>
      <div className="control-deck">
        ...
        <div className="tools"><button className={`voice ${props.compressor ? "on" : ""}`} onClick={props.onToggleCompressor}><i /> 人声增强</button><label className="speed"><span>播放速度</span><input type="range" min="0.5" max="3" step="0.05" value={props.speed} onChange={event => props.setSpeed(Number(event.target.value))}/><b>{props.speed.toFixed(2)}×</b></label></div>
      </div>
    </section>
  );
}
