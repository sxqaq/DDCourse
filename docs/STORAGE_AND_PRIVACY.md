...
桌面端另外维护“文档/DDCourse/学习笔记.json”，启动时与 localStorage 按每条记录的 `updatedAt` 合并。删除操作会保留 tombstone，避免尚未完成磁盘写入时退出导致旧记录在下次启动时复活。它不包含视频或播放进度。读取和写入均经过运行时结构校验；旧记录缺少 `updatedAt` 或删除标记时会自动迁移。
